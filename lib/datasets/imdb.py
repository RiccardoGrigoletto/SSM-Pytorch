...
class Imdbs(imdb):
    ...
    def roidb(self):
        ...
        for k,v in self.imdb_dict.items():
            roidbs.extend(v.roidb)
        return roidbs
    @property
    def num_images(self):
        nums = 0
        for imdb in self.imdb_dict.values():
            nums += imdb.num_images
        return nums
    ...
    def classes(self):
        classname = set()
        for k,v in self.imdb_dict.items():
            classname = classname.union(v.classes[1:])
        return ['__background__']+list(classname)
    ...
    def replace_gt(self,ss_candidate,ss_fake_gt,flipped):
        '''
         if ss is empty,there maybe some problems
        '''
        ...
        for imdb in self.imdb_dict.values():
            ...
            while position<= len(ss_candidate):
                if position == len(ss_candidate) or ss_candidate[position]>=prefix_num_sum+imdb.num_images:
                    ...
                    ss_fake_gt = ss_fake_gt[position:]
                    prefix_num_sum += imdb.num_images
                    break
                position += 1
