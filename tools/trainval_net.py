...
def parse_args():
  ...
  """
    ...
    parser.add_argument('--image_validation', dest='image_val', help='method for image validation: ICV, LS, LT, MS',
                        default='MC', type=str)
    ######################### end ##################################
    if len(sys.argv) == 1:
        parser.print_help()
...
def get_Imdbs(imdb_names):
    ...
    for imdb in imdbs:
        imdb.set_proposal_method(cfg.TRAIN.PROPOSAL_METHOD)
        print('set proposal method:{:s}'.format(cfg.TRAIN.PROPOSAL_METHOD))
    if imdb_names == 'train_TASK2_30objs_TRA_supervised+train_TASK2_30objs_NoTRA_unsupervised':
        return datasets.imdb.Imdbs(imdbs, name='icwt')
    else:
...
if __name__ == '__main__':
    ...
    while (True):
        ...
        ss_avg_score = []
        print("time elapsed for detection of the unlabeled images: {}".format(time.time() - t0))
        ...
        for i in range(len(unlabeledidx)):
                ...
                if v:
                    if (np.sum(y == 1) == 1 and np.where(y == 1)[0] != 0):
                        ...
                        if args.image_val == "ICV":
                            validate, avg_score = image_cross_validation(net, roidb, labeledsample, curr_roidb, pre_box,
                                                                         pre_cls)
                        if args.image_val == "LS":
                            validate, avg_score, _ = localization_stability(net, roidb, labeledsample, curr_roidb,
                                                                            pre_box, pre_cls)
                        if args.image_val == "LT":
                            validate, avg_score, box = localization_tightness(net, roidb, labeledsample, curr_roidb,
                                                                              pre_box,
                                                                              pre_cls, 0.15)
                        if args.image_val == 'MS':
                            box_scores.append(margin_sampling(boxscore))
                        if args.image_val == 'MC':
                            box_scores.append(max_confidence(boxscore))

                        if args.image_val != 'MS' and args.image_val != 'MC':
                            if validate:
                                im_boxes.append(box)
                                im_cls.append(np.where(y == 1)[0])
                                if args.image_val == "ICV" or args.image_val == "LT":
                                    ss_accum_score += float(avg_score)
                                elif args.image_val == "LS":
                                    ss_accum_boxes += np.max(scoreMatrix[i])
                                    ss_accum_score += np.max(scoreMatrix[i]) * float(avg_score)
                            else:
            ...
            if validate:
                for j, box in enumerate(boxRecord[i]):
                    im_boxes.append(box)
                    im_cls.append(np.where(y == 1)[0])
                ss_accum_score += float(image_score)
            else:
                al_candidate_idx.append(unlabeledidx[i])
        ...
        if args.enable_ss:
            ...
            print('ss_num:', ss_num, 'ss_candidate_idx:', len(ss_candidate_idx), 'ss_proportion_checkpoint:',
                  ss_proportion_checkpoint[0])
            if ss_num + len(ss_candidate_idx) >= ss_proportion_checkpoint[0]:
                ...
                print('ss_proportion_checkpoint: {}%% samples for al, model name:{}'.format(tmp / initial_num,
        ...
        logging.info(
            'last model name :{},AL amount:{}/{},al_factor:{},SS amount: {}/{},ss_factor:{},total_process_num:{}'.format(
                pretrained_model_name, al_num, initial_num, al_factor, ss_num, initial_num, ss_factor,
                total_process_num))
        ...
        next_train_idx = [x + initialnum for x in next_train_idx if x >= initialnum] + [x + initialnum + remainnum for x
                                                                                        in next_train_idx if
                                                                                        x >= initialnum]
